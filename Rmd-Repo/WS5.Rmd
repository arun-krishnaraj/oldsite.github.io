...
Let's review dnorm(), pnorm(), qnorm()

```{R}
#review
ggplot()+geom_line(aes(x=c(-4,4)),
stat = "function", fun = dnorm)+xlab("z")
```

Random sampling functions!

```{R}
norms <- rnorm(5000) #5000 draws from a standard normal
unifs <- runif(5000) #5000 draws from a uniform(0,1)

hist(norms)
hist(unifs)
```

Use uniform samples to generate random samples from ANY DISTRIBUTION!

```{R}
hist(unifs) #they appear uniform

hist(qnorm(unifs)) #give these to qnorm, it will spit out z-scores cutting off each "probability"! 

#This should look roughly the same as hist(norms)
```
...
```{R}
txhousing %>% filter(city == "Austin" & year == max(year))
...
```{R}
txhousing %>% select(city, ends_with("s"))
```
