...
**This homework is due on Sep 13, 2020 at 11:59pm. Please submit as an HTML file on Canvas.**
...
There are 50 distinct chicks, 12 distinct time points, 4 distinct diet conditions. There is an average of 12.5 chicks per diet condition at the start of the experiment; there were 20 chicks in diet condition 1, and 10 chicks in each of diet condition 2, 3, and 4.
...
This plot provides a clearer depiction of the effects of each diet on chick weight over time; none of the prior plots provide information on chick weight behavior for each diet, which makes it difficult to interpret the impact the diet has. Depending on what the goal of these diets is, we could easily choose the facet that visually reflects this.
...
The linear regressions illustrate more clearly the effect each diet has on weight over time. While the effect of diet seems to only become apparently after some time, the chicks in diets 3 and 4 gained more weight per day than those in diet 1 and 2, as illustrated by the steeper regression slope.
...
```{R}
library(ggplot2)
ggplot(ChickWeight, aes(x=Diet, y= weight)) + geom_bar(stat = "summary", aes(fill = Diet)) + scale_fill_brewer() +
