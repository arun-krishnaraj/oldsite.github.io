...
- IMF's arise from electrostatic interactions
...
- solubility: ability of solute to dissolve in solvent
  - water interacts with polar substances, dissolves electrolytes
    ...
    - dielectric constant: ability of solvent to dissolve, diminish electrostatic interactions
...
- covalent bonds: two atoms share a pair of electrons, equally or unequally based on electronegativity (polar or nonpolar)
...
$$H_2O + H_2O \leftrightarrow  OH^- + H_3O^+$$ (auto ionization)
$$K_w= [H_3O^+][OH^-] = 1*10^{-14}$$
...
$$HA \text{ (conjugate acid)} \rightarrow H^+ + A^- \text{ (conjugate base)}$$
...
$$pH = pKa + log \frac{\text{[conjugate base]}_f}{\text{[conjugate acid]}_f}$$
...
- inflection point: when half of acid is neutralized (pH = pKa for weak acids)
  - -> pH = pKa, [CB] = [CA]
...
- cells are differentiated by membrane bilayers; hydrophilic heads interact with the environment, while hydrophobic tails interact with each other
...
- quaternary structure: only exists when multiple polypeptide chains are assembled into a single protein; composed of any number of subunits (like $\alpha_2 \beta_2$ tetramer of human hemoglobin)
...
- 20 kinds of side chain change the character of aminos (sometimes conferred by functional groups); can be classified by R groups
  ...
  - Positively charged amino acids (R groups hold positive charge at pH $\approx$ 7.4), highly hydrophilic
    ...
    ![](C:\Users\arunk\Documents\GitHub\arun-krishnaraj.github.io\images\Positive.JPG)
...
- amide covalent linkages between amino acids in a polypeptide chain; endergonic dehydration synthesis reactions
...
- free energy determines if a reaction can take place
  - only negative $\Delta G$ reactions are spontaneous (exergonic)
    - posititive $\Delta G$ reactions are endergonic
...
- HbA: normal adult hemoglobin, 2 $\alpha$ and 2 $\beta$ subunits
  - HbS: sickle hemoglobin, point mutation on $\beta$ chain; causes precipitation due to hemoglobin polymerization 
...
- monosaccharides are aldehydes or ketones with two or more alcohol groups (D saccharides are biologicaly active, hydroxyl on right)
  ...
  ![](C:\Users\arunk\Documents\GitHub\arun-krishnaraj.github.io\images\carb-isomers.JPG)
  - cyclic carbs can be $\alpha$ (C-1 hydroxyl down) or $\beta$ (C-1 hydroxyl up)
- fructose can be in either 5 or 6 member cyclic forms (furanose or pyranose)
- glucose can react with oxidizing agents when in open form (prevents cyclic binding)
- oligosaccharides contain monosaccharides linked by O-glycosidic bonds (anomeric carbon of glucose and hydroxyl group)
  - polymerization structure influences which carbs can be digested
- glycoproteins consist of proteins attached to carbohydrates
  - glyoprotein: protein is largest component; membrane functions
  - proteoglycan: protein is attached to glycosaminoglycan, mostly carbohydrate; strutural or lubricant role
    - glycosaminoglycans have large repeating disaccharide units
  - mucoprotein: mostly carbohydrate, often lubricants
- linkage is normally to nitrogen in asparagine R group, or oxygen in serine or threonine R groups

Lipids Video 4

- Fatty acids: carboxylate joined with hydrocarbon chain
  - referred to by number of carbons and double bonds present 
  - Omega or Delta nomenclature can be used to convey position of double bonds (from ending or from starting carbons)
  - saturated fatty acids solidify easier, have high melting temperature
- triacylglycerol: energy storage molecule, holds onto fatty acid chains
- waxes: formed when long chain alcohol and fatty acid join with an ester bond
  - fats have unique aqueous configurations based on solubility 
- types of membrane lipids:
  - phospholipids: contain 2 fatty acids, platform, phosphate, and an alcohol 
    - platforms are usually glycerol or sphingosine 
  - glycolipids are carb containing lipids, with carb on extracellular surface of cell membrane where they play a role in cellular interactions
  - steroids: built on tetracyclic platform: cholesterol maintains membrane fluidity and is a precursor to hormonal steroids 
  
Clutch Protein Ligand

- ligand: any small substance that can reversibly bind with a larger biomolecule
- the higher the rate constant, the reaction is likely faster
  - ligands have rates of association and dissociation $k_a, k_d$ for forming PL and breaking PL

Clutch Ligand Equilibrium 

- association and dissociation rate constants $(K_a, K_d)$
  - the greater the $K_a$, the stronger the affinity for ligand
  - $K_a$ has units of $M^{-1}$ and is the reciprocal of $K_d$; $K_a = \frac{[PL]}{[P][L]} = \frac{k_a}{k_d} = \frac{1}{K_d}$
- $K_d$ is more commonly used, and has a similar interpretation to $K_m$; both inversely represent binding affinity
  - $K_d = [L]$ allows half of the binding sites to be occupied

Clutch fractional saturation

- Y: fraction of occupied ligand binding sites
  - saturation curves plot $\theta$ against $[L]$, we can use this to find the $K_d$
    - $\theta  = \frac{[L]}{[L]+K_d}$
...
- hemoglobin exhibits cooperative effect characteristic of allosteric enzymes; allows full saturation in the lungs and transport to tissue

9.2 Myoglobin and Hemoglobin bind oxygen in Heme groups

- both myoglobin and hemoglobin rely on prosthetic heme group to bind to oxygen; central Iron is stabilized by 4 charged nitrogens, as well as histidine residue;
  - the last Iron coordination site remains open to bind to oxygen; tightens Iron electron cloud to flatten porphyrin ring
  
9.3 Hemoglobin binds oxygen cooperatively 

- heme consists of 2 $\alpha$ and 2 $\beta$ subunits, similar to those of myoglobin
  - oxygenation results in a T to R conversion, central pore is held tighter
  
9.4 An allosteric regulator determines the oxygen affinity of hemoglobin

- 2,3-BPG reduces hemoglobin oxygen affinity to increase delivery to tissues
  - fits in the central pore of T hemoglobin, leads to release of oxygen and T form stabilization
  
9.5 Protons and carbon dioxide promote the release of oxygen

- hydrogen ions and carbon dioxide, byproducts of metabolism, enhance oxygen release 
  ![](C:\Users\arunk\Documents\GitHub\arun-krishnaraj.github.io\images\heme_ion.JPG)
  - creation of stabilizing salt bridges and carbamate reduce oxygen affinity 
    ![](C:\Users\arunk\Documents\GitHub\arun-krishnaraj.github.io\images\heme_co2.JPG)

9.6 Heme subunit mutation can lead to disease 

- point mutations decreases solubility of deoxygenated hemoglobin, results in a high concentration of deoxygenated Hb
  - sickles nearby cells by reducing oxygen concentration in narrow blood vessels 
  
Chapter 10

10.1 Monosaccharides are the simplest carb

- aldehydes or ketones with at least two hydroxyl groups
  - can be a ketose or aldose (if they have a keto or aldehyde group)
- carbs occur in a large number of isomeric forms; chiral centers contribute diastereoisomers
- carbs often react to form cyclic forms, which can be alpha or beta form depending on orientation of hydroxyl bound to carbon 1
- glycosidic bonds: 
  - O-glycosidic: bond between anomeric carbon of glucose and oxygen of alcohol
  - N-glycosidic: bond between anomeric carbon of glucose and nitrogen of amine
  
10.2 Monosaccharides are linked to form complex carbs

- oligosaccharides are built by linkage of two or more monosaccharides by O-glycosidic bonds
  - actual bond chemistry depends on type of ring configuration and carbon position
- glycogen and starch are storage forms of glucose
  - polysaccharides play important roles in energy and structure
  - glycogen is used for energy storage in animal cells; starch (amylopectin and amylose) are the storage form for plants
- cellulose is a structural carb, unbranched glucose joined by $\beta$-1,4 linkages instead of the $\alpha$-1,4 linkages
  - straight chain form can't be digested by mammals (still improves digestion)
  
10.3 Carbs are attached to proteins to form glycoproteins 

- sugars are attached to amide nitrogen in asparagine, or hydroxyl oxygen in serine or threonine (largely protein by weight)
- proteoglycans: more carb by weight, function as lubricants and structural components, mediate cellular adhesion
  - human blood groups are based on glycosylation patterns
  
Chapter 11 Lipids

11.1 Fatty Acids

- chains of hydrocarbons with terminal carboxylic acid groups
  - act as fuel and building blocks for membrane lipids
  - formal name is based on parent hydrocarbon name
  - fatty acids with one or more double bonds are unsaturated, no double bonds is saturated
  - n:m notation has n carbons with m double bonds
  - carbon atoms are numbered starting at carboxyl terminus (Delta system, but can be reversed for the w system)
  - unsaturated fats have lower melting points than saturated ones
    - chain interaction (van der Whaal) is made more difficult when chains aren't straight
  - shorter chains also have lower melting points

#### Week 5 Lectures

U2W5V1

- higher protein membrane content than commonly portrayed (25% - 80%), spontaneous membrane formation is driven by hydrophobic effect
  - carb side chains of glycolipids are usually outward facing
  - integral and peripheral membrane proteins
- cholesterol disrupts straight chain bonding; can also form complexes with lipids and proteins (lipid raft to speed up chemical processes)
- proteins have been shown to diffuse across the membrane; lipids can diffuse laterally or transverse (fast vs slow)
  - phospholipids are usually asymmetrically distributed across the bilayer
- integral proteins cross the bilayer; peripheral proteins are somehow attached to a single side
  - bacteriorhodopsin: series of alpha helices spanning membrane (most common motif)
    - bilayers are generally very thin (10 nm, about 66 amino acid long alpha helices)
  - bacterial porin: beta strands form a central pore (transmembrane barrel), less selective transport
  - peripheral proteins can contribute to binding of signalling enzymes (prostaglandin requires a fatty acid to become activated)
- membrane transport processes
  - nonmediated transport: diffusion across a membrane, can be fast for hydrophobic or polar (fast or very slow respectively)
    - movement from higher to lower gradient, no energy input required
  - facilitated (passive) transport: diffusion of solutes is accelerated by transporter proteins (pores, channels, carriers)
  - active transport requires energy to transport molecules against their gradient 
- uniport: one molecule moving
- symport: two different molecules moving in the same direction
- antiport: two differenet molecules moving in opposite directions 
- channel vs pump
  - with vs against gradient
  - potassium channel selects against sodium: carbonyl interior forces potassium to peel away from hydration shell, sodium isn't large enough to form bonds with carbonyls 
  
U2W5V2

- we can use free energy calculations to determine if molecules will move across a bilayer
  - $\Delta G^o$ is 0 for diffusion across a semi-permeable membrane $\rightarrow \Delta G = R \ T \ ln(Q)$, where $Q = \frac{Final}{Initial}$
- membrane potential: associated change in free energy as charged species cross potential gradient
????????????????????????? need to review this $$\Delta G _t = RT ln(C_2/C_1) + Z F \Delta \Psi$$
  - most cells have more anions inside the cell, creates a voltage (charge buildup across bilayer)

U2W5V3

- refer to net change in free energy to determine if a symport or antiport transport process is spontaneous 
- secondary transport: nonspontaneous transport mediated by an active transporter establishing a gradient 
- sodium potassium antiporter: requires external energy, for every ATP used 3 sodium are pumped out and 2 potassium are pumped in (both against their concentration gradients)
   - hydrolysis of ATP has a large negative free energy change: spreading out of negative charges, increased entropy, more stable inorganic phosphate form due to resonance
   - chemical energy from energy is transferred into mechanical energy (phosphate drives a conformationsl change in the pump protein)
   - 1. ATP binds to the ATP binding site, allowing sodium to enter the pump
   - 2. phosphate remains bound to aspartate residue, ADP release causes pump to open outside
   - 3. Sodium is released and potassium enters the pump
   - 4. ATP binds and knocks off phosphate
   - 5. inside opens and potassium enters

#### Week 6 Lectures

U2W6V1

- primary messengers can generate intracellular secondary messenger, which can then activate a signal cascade until signal ceases
  - hormones: chemical messengers, cannot easily pass through membrane (except steroids)
  - photons: signalling proteins can pick up light via conformational changes
  - neurotransmitters: small molecule transmitting nerve signals
- transmembrane receptor protein changes conformation to transfer signal
  - 7 transmembrane-helix receptor: consists of 7 alpha helices, activates a G protein (GTP or GDP bound) when signaled; causes activation of an interior protein kinase
    - subunits separate from GDP, GTP replaces it; GTP subunit binds to adenylate cyclase to generate cyclic AMP and turn on kinase (phosphorylate another substrate)
    - Adenylate cyclase is turned off by phosphorylating GTP to GDP, which forces subunit reassociation and freeing from adenylate cyclase
    - cAMP also resets the kinase as it's spontaneously converted back to AMP
    - as concentration of primary messenger decreases, it spontaneously dissociates and switches off the receptor 
  
U2W6V2

- Dimeric membrane receptors that recruit protein kinases
  - quaternary structural change instead of tertiary drives mechanism
  - receptor dimerizes once signal factor binds; cross phosphorylation results in activation of kinase, enables further phosphorylation 
- dimeric protein receptors that are protein kinases 
  - single integral protein has both receptor and kinase function; receptor brings entire subunits together, phosphorylates and activates another signaling G protein 
  
U2W6V3

- digestion: set of biochemical reactions to convert food into biologically useful building blocks
  - begins in mouth, food is chewed and mixed with saliva (water + mucoproteins, alpha amylase and ions)
  - stomach pump uses energy to create acidic environment, denatures consumed proteins
    - peptide backboen is more exposed, can be more easily hydrolysed into oligopeptides
  - stimulates reduction of acidic conditions by pancreas so other enzymes can enter; gallbladder releases bile to digest lipids
    - gastric and pancreatic enzymes hang out as zymogens until activated by proper conditions 
  - digestion of proteins
    - oligopeptides are broken into amino acids, absorbed into intestinal cells and pushed through antiport into blood
  - monosaccharide digestion
    - Galactose + Glucose symport transport with sodium, Fructose uniport into intestinal cell; uniport into blood stream
  - lipid digestion
    - need to account for hydrophobicity; bile makes lipids more soluble through detergent activity 
    - cholesterol as precursor for bile, use of fiber to reduce cholesterol levels
    - micelle formation, transport to intestinal cells and lymph systems 

U2W6V4

- basic principles of energy manipulation
  - use of metabolic pathways to control energy flow
  - ATP is the energy currency of life, and is used to drive nonspontaneous processes forward
  - ATP can be formed by the oxidation of carbon fuels
  - a limited number of reaction types with common intermediates 
  - metabolic pathways are highly regulated
  - enzymes are organized into large complexes 
- catabolic pathways combuts carbon fuels to synthesize ATP or create ion gradients
- anabolic pathways use ATP to synthesize larger biomolecules 
- exergonic ATP hydrolysis can be used to drive endergonic reactions 
  - energy stored in high energy phosphate bonds; breaking of ATP results in
    - freeing of negative charge repulsion
    - resonance stabilization increases
    - increase in entropy 
    - stabilization by hydration
- Phosphoryl-transfer potential: standard free energy of hydrolysis of a phosphate group (more negative is more prone to transfer/higher energy when transferred)
- exercise depends on ATP generation; creatine phosphate regenerates ATP from ADP, allowing for short bursts; longer stretches must use other metabolic pathways

U2W6V5

- recurring theme of activated carriers
  - carriers are kinetically stable in absence of specific catalysts
  - metabolism of activated groups is accomplished with few carriers 
- ATP can be considered a carrier for phosphate groups, used to activate or tag many groups
- NAD$^+$ and NADP$^+$, differ in presence of phosphate group
  - differ in where they carry electrons
- many activated carriers are derived from vitamins (B vitamins serve as coenzymes)

Chapter 13 Signal Pathways

13.1 Signal Transduction Depends on Molecular Circuits

- all signal circuits contain
  - release of primary messenger
  - reception of primary messenger: membrane receptors transfer information into the cell using intra and extracellular domains; binding to ligand alters the structure of the receptor
  - relay of information by the second messenger: receptor changes lead to changes in second messenger concentrations
  - activation of effectors that directly alter the physiological response: activation of inhibition of tools controlling metabolism, gene activation, and nerve transmision
  - termination of the signal
  
13.2 Receptor Proteins Transmit Information into the cell

- seven transmembrane helix receptors
  - consists of 7 helices spanning the membrane
  - ligand binding causes a conformation change in 7TM receptors
  - G protein: coupled to ligand binding, transmits signal to interior of cell
    - unactivated GDP bound to inactive $\alpha$ and $\gamma$ subunits is replaced by active GTP by ligand binding
  - G proteins can bind to adenylate cyclase, increasing cAMP production 
    - increased cAMP concentration can activate protein kinases
    - PKA (protein kinase A) consists of two regulatory and two catalytic subunits that are inactive in absence of cAMP; cAMP binding to regulatory subunits frees the catalytic subunits to perfom enzymatic phosphorylation
    - $\alpha$ subunits of G have GTPase activity, converting it into GDP and free phosphate; acts as a clock to reset the GDP-bound form after some time
  - ligand can also dissociate based on concentration (reversible binding)
  - phosphoinositide cascade: intracellular messengers made from cleavage of membrane protein into IP$_3$ and diacylglycerol; IP$_3$ causes a release of calcium ions into the cytoplasm, while Diacylglycerol + calcium activates a protein kinase  
  
13.3 Some Receptors Dimerize in response to ligand binding 

- receptors have a single membrane-spanning helix; ligand binds to two receptors to activate it 
  - JAK2 receptors phosphorylate each other upon ligand binding, and are then free to phosphorylate other molecules 
  - Ras is part of the small G protein family, which assists in almost all cell functions
    - cycle between active GTP and inactive GDP-bound forms as well
  
Chapter 14 Digestion

14.1 Digestion prepares large biomolecules

- proteins, fats, carbs are digested into their basic components by proteases, lipases, and carb digestive enzymes 
- almost all digestive enzymes are secreted in inactive form (zymogen); activate d by proteolytic cleavage when signaled
  
14.2 Proteases digest proteins 

- proteins are denatured in the stomach, then pepsin begins to break them into protein fragments
  - acidic environment is generated by gastric proton pumps 
- small intestine releases secretin, which stimulates neutralizing sodium bicarbonate release
  - cholecystokinin is released by intestinal cells, which stimulates other digestive enzymes from the pancreas
  - oligiopeptides are broken into animo acids by peptidases attached to intestinal surface, which are then transported into intestinal cells
  
14.3 $\alpha$ Amylase digests carbs

- unique carbohydrates are digested by corresponding enzymes, until they become monomers
  - glucose and galactose are transported into intestinal cells with a symport channel, fructose diffuses through GLUT5 transporter; these then move through GLUT2 into the bloodstream
  
14.4 Lipid digestion depends on hydrophobicity

- lipids must be degraded to fatty acids for absorption
- lipids converted into emulsion in stomach, after which bile salts are added; pancreatic lipases produce fatty acids and mono acylglycerol, which are carried in micelles to be absorbed by intestinal cells
  - move into cells by FABP, then moved to smooth ER and release to lymph system and blood
  
Chapter 15 Metabolism

15.1 Energy is required for fundamental needs

- living organisms require free energy to
  - perform mechanical work in cellular movement
  - actively transport molecules
  - sythesize macromolecules
  
15.2 Metabolism has many interconnected reactions

- metabolism is a series of chemical reactions that converts a biomolecule in a defined fashion; can convert energy to useful forms, or require energy inputs to proceed (catabolic vs anabolic)
  - irreversible reactions of pathways are always distinct
- overall free energy change is the sum of changes for coupled reactions; thermodynamically unfavorable processes can be made favorable by coupling
  
15.3 ATP is the currency of free energy

- free energy is converted to ATP, which then drives anabolic processes
  - ATP is energy rich due to triphosphate unit; strong charge repulsions are freed when hydrolyzed to ADP and AMP
  - large phosphoryl-transfer potential of ATP is due to:
    - electrostatic repulsion
    - resonance stabilization of orthophosphate compared to bound phosphate
    - increase in entropy
    - products are stabilized by hydration
  
15.4 Oxidation is a source of cellular energy

- ATP is not a long-term storage form of energy (high ATP turnover, generated by catabolism)
  - carbon is oxidized to $\text{CO}_2$ and ATP is regenerated (electrons lost and gained, oxidation-reduction reactions); oxidation occurs one carbon at a time
- electrons are captured by NAD$^+$, and high phosphoryl tranfer-potential drives ATP regeneration (trapped as high-phosphoryl transfer potential intermediate)
  
15.5 Metabolic pathways have recurring motifs

- activated carriers of electrons for oxidation
  - electrons are transferred between carriers before oxidizing O$_2$ 
  - NAD$^+$ is a major electron carrier in oxidation of fuel; nicotinamide ring is reduced to NADH
- activated carriers of electrons of the synthesis of biomolecules
  - reductive biosynthesis: reduction of precursors in anabolism
  - NADPH is used for reductive biosynthesis; NADH for ATP generation
- activated carrier of two-carbon fragments
  - CoA: carries acyl groups, derived from pantothenate; high acetyl group transfer potential 
- all are kinetically stable in absence of catalysts, enable control of free energy generation; many active group interchanges are done by small set of carriers
- many activated carriers are derived from vitamins: niacin, riboflavin, pantothenate, other B vitamins
  - other vitamins serve non-coenzyme functions and are needed for proper function
  
15.6 Metabolic processes are regulated in 3 ways

- control of amount of enzymes: adjusted by rate of transcription of genes encoding them
- their catalytic activities: allosteric control, feedback inhibition, reversible covalent modification
  - hormones coordinate metabolic relations, often through reversible modification of enzymes
  - energy charge: measure of energy status of the cell $\text{energy charge} = \frac{\text{[ATP] + 1/2[ADP]}}{\text{[ATP]+[ADP]+[AMP]}}$
    - high energy charge inhibits catabolic pathways, stimulates anabolism; usually buffered near $0.85$
    - can also consider the phosphorylation potential $= \frac{\text{[ATP]}}{\text{[ATP]+[P}_i]}$
- accessibility of substrates
  - regulation and flexibility enhanced by compartmentalization (separate opposed reactions)
